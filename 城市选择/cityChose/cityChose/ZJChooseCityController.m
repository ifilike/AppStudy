...
@interface ZJChooseCityController ()<UISearchBarDelegate,CLLocationManagerDelegate>
/**
 * 定位城市ID
 */
...
@implementation ZJChooseCityController
...
-(void)createSearch{
    ...
    self.searchBar.placeholder = @"城市名称或者首字母";
    ...
    [self.tableView registerClass:[ZJCityGroupCell class] forCellReuseIdentifier:cityGroupCell];[self.tableView registerClass:[ZJCityGroupCell class] forCellReuseIdentifier:cityCell];
    [self.tableView registerClass:[ZJCityHeaderView class] forHeaderFooterViewReuseIdentifier:cityHeaderView];
}
...
-(void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    if (searchText.length == 0) {
        ...
        self.isSearch = YES;
        for (ZJCity *city in self.recordCityData) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    ZJCityGroup *group = [self.cityDatas objectAtIndex:section - 3];
    return group.arrayCitys.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.isSearch) {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
        ZJCity *city = [self.searchCities objectAtIndex:indexPath.row];
        [cell.textLabel setText:city.cityName];
        return cell;
    }
    
    if (indexPath.section < 3) {
        ZJCityGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cityGroupCell];
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    ZJCityGroup *group = [self.cityDatas objectAtIndex:indexPath.section - 3];
    ZJCity *city = [group.arrayCitys objectAtIndex:indexPath.row];
    [cell.textLabel setText:city.cityName];
    return cell;
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        return [ZJCityGroupCell getCellHeightOfCityArray:self.localCityData];
    }else if (indexPath.section == 1){
        return [ZJCityGroupCell getCellHeightOfCityArray:self.commonCityData];
    }
    else if (indexPath.section == 2){
        return [ZJCityGroupCell getCellHeightOfCityArray:self.hotCityData];
    }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    ZJCityHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:cityHeaderView];
...
-(void)didSelectedCity:(ZJCity *)city{
...
#pragma mark ZJCityGroupDelegate
...
-(NSMutableArray *)localCityData{
    if (!_localCityData) {
        ...
        if (self.locationCityID != nil) {
            ZJCity *city = nil;
            for (ZJCity *item in self.recordCityData) {
...
-(NSArray *)hotCitys{
    if (!_hotCitys) {
        _hotCitys = @[@"100010000",@"200010000",@"300110000",@"300210000"];
    }
    return _hotCitys;
}
-(NSMutableArray *)cityDatas{
    if (_cityDatas == nil) {
        ...
        for (NSDictionary *groupDic in array) {
            ZJCityGroup *group = [[ZJCityGroup alloc] init];
...
-(void)cancelButtonDown:(id)sender{

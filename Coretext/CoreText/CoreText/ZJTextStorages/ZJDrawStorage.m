...
- (void)setAppropriateAlignment
{
    ...
    if (_size.height <= _fontAscent + _fontDescent) {
        _drawAlignment = ZJDrawAlignmentCenter;
    }
}
...
{
    // 添加文本属性和runDelegate
    [attributedString addAttribute:kZJTextRunAttributedName value:self range:range];
...
CGFloat TYTextRunDelegateGetAscentCallback( void *refCon ){
    NSLog(@"%@",refCon);
    ZJDrawStorage *drawStorage = (__bridge ZJDrawStorage *)refCon;

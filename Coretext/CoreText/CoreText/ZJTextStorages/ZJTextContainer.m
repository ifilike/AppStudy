...
@implementation ZJTextContainer
...
- (void)addTextStoragesWithAtrributedString:(NSMutableAttributedString *)attString
{
    if (attString && _textStorageArray.count > 0) {
        ...
        for (id<ZJTextStorageProtocol> textStorage in _textStorageArray) {
            
            // 修正图片替换字符来的误差
            if ([textStorage conformsToProtocol:@protocol(ZJDrawStorageProtocol) ]) {
                continue;
            }
            
            if ([textStorage conformsToProtocol:@protocol(ZJLinkStorageProtocol)]) {
                if (!((id<ZJLinkStorageProtocol>)textStorage).textColor) {
                    ((id<ZJLinkStorageProtocol>)textStorage).textColor = _linkColor;
                }
            }
        ...
        for (id<ZJTextStorageProtocol> textStorage in _textStorageArray) {
            ...
            if ([textStorage conformsToProtocol:@protocol(ZJDrawStorageProtocol)]) {
                id<ZJDrawStorageProtocol> drawStorage = (id<ZJDrawStorageProtocol>)textStorage;
...
{
    [textStorageArray sortUsingComparator:^NSComparisonResult(id<ZJTextStorageProtocol> obj1, id<ZJTextStorageProtocol> obj2) {
...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    for (int i = 0; i < numberOfLines; i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            id<ZJTextStorageProtocol> textStorage = [attributes objectForKey:kZJTextRunAttributedName];
            ...
            if (textStorage) {
                ...
                } else if ([textStorage conformsToProtocol:@protocol(ZJLinkStorageProtocol)]) {
...
- (void)addRunRectDictionary:(NSDictionary *)runRectDictionary
{
    if (runRectDictionary.count < _runRectDictionary.count) {
        ...
        // 遍历不会画出来的
        for (id<ZJTextStorageProtocol>drawStorage in drawStorageArray) {
            if ([drawStorage conformsToProtocol:@protocol(ZJViewStorageProtocol)]) {
                [(id<ZJViewStorageProtocol>)drawStorage didNotDrawRun];
            }
        }
    }
...
- (void)enumerateDrawRectDictionaryUsingBlock:(void (^)(id<ZJDrawStorageProtocol> drawStorage, CGRect rect))block
{
    [_drawRectDictionary enumerateKeysAndObjectsUsingBlock:^(NSValue *rectValue, id<ZJDrawStorageProtocol> drawStorage, BOOL * stop) {
        if (block) {
...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<ZJTextStorageProtocol> textStorage))successBlock
{
    return [self enumerateRunRect:_runRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:successBlock];
}

- (BOOL)enumerateLinkRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<ZJLinkStorageProtocol> textStorage))successBlock
{
    return [self enumerateRunRect:_linkRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:successBlock];
}

- (BOOL)enumerateRunRect:(NSDictionary *)runRectDic ContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<ZJTextStorageProtocol> textStorage))successBlock
{
    ...
    // 遍历run位置字典
    [runRectDic enumerateKeysAndObjectsUsingBlock:^(NSValue *keyRectValue, id<ZJTextStorageProtocol> textStorage, BOOL *stop) {
        
        CGRect imgRect = [keyRectValue CGRectValue];
        CGRect rect = CGRectApplyAffineTransform(imgRect, transform);
        
        if ([textStorage conformsToProtocol:@protocol(ZJDrawStorageProtocol) ]) {
            rect = UIEdgeInsetsInsetRect(rect,((id<ZJDrawStorageProtocol>)textStorage).margin);
        }
...
@implementation ZJTextContainer (Add)

- (void)addTextStorage:(id<ZJTextStorageProtocol>)textStorage
{
    if (textStorage) {
...
{
    if (textStorageArray) {
        for (id<ZJTextStorageProtocol> textStorage in textStorageArray) {
            if ([textStorage conformsToProtocol:@protocol(ZJTextStorageProtocol)]) {
...
@implementation ZJTextContainer (Append)
...
- (void)appendTextStorage:(id<ZJAppendTextStorageProtocol>)textStorage
{
    if (textStorage) {
        if ([textStorage conformsToProtocol:@protocol(ZJDrawStorageProtocol)]) {
            [(id<ZJDrawStorageProtocol>)textStorage setTextfontAscent:_font.ascender descent:_font.descender];
        } else if ([textStorage conformsToProtocol:@protocol(ZJLinkStorageProtocol)]) {
            if (!((id<ZJLinkStorageProtocol>)textStorage).textColor) {
                ((id<ZJLinkStorageProtocol>)textStorage).textColor = _linkColor;
            }
...
{
    if (textStorageArray) {
        for (id<ZJAppendTextStorageProtocol> textStorage in textStorageArray) {
            if ([textStorage conformsToProtocol:@protocol(ZJAppendTextStorageProtocol)]) {

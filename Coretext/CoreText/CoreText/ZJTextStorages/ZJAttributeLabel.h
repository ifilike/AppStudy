...
typedef NS_ENUM(NSInteger,ZJVerticalAlignment) {
    ZJVerticalAlignmentTop,
    ZJVerticalAlignmentCenter,
    ZJVerticalAlignmentBottom,
};
@class ZJAttributeLabel;
@protocol ZJAttributeLabelDelegate <NSObject>
...
/**
 *  ZJAttributedLabel 属性文本 支持图文混排显示，支持添加image和UIView，支持自定义排版
 */
@interface ZJAttributeLabel : UIView

@property(nonatomic,assign) id<ZJAttributeLabelDelegate> delegate;
...
@property (nonatomic, assign)   ZJVerticalAlignment verticalAlignment; // 垂直对齐方式 默认是向上对齐
...
@interface ZJAttributeLabel (AppendAttributedString)
/**
 *  追加(添加到最后) 普通文本
 *
 *  @param text 普通文本
...
@interface ZJAttributeLabel (Link)
...
@interface ZJAttributeLabel (UIImage)
...
- (void)addImage:(UIImage *)image range:(NSRange)range
            size:(CGSize)size
       alignment: (ZJDrawAlignment)alignment;
